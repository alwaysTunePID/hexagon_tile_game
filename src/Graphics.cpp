...
Graphics::Graphics(unsigned seed)
    : clock{}, worldObjectSprites{}, tilesprites{}, tileCosmetics{}, grass{ WorldObjectType::grass, 0},
      ...
      m_timePastForShader{}, m_globalLightVec{}, generator{ std::default_random_engine{seed} }, particleSystem{}
{
...
void Graphics::updateAndDrawParticleSimulation(sf::RenderWindow& window, displayInput& input, double dt)
{
    if (input.enableShaders)
    {
        ...
        particleSystem.fadeParticleTrail(dt);
        particleSystem.updateFlowField((float)dt);
        particleSystem.updateParticles(dt);
        //particleSystem.blurParticles(dt);
        particleSystem.updateBall(dt);
        particleSystem.updatePosAndScale(camera);
        particleSystem.draw();
    }
}